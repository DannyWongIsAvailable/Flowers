...
def organize_and_split_dataset(src_dir='../src/train',
                               ...
                               train_dir='../datasets/flowers_cls/train',
                               val_dir='../datasets/flowers_cls/val',
                               val_ratio=0.1,
                               seed=42):
    """
    划分数据集：所有图片复制到train，随机选择部分复制到val

    策略：
    - 100%的图片复制到train目录
    - 随机选择val_ratio比例的图片复制到val目录（从train中采样）
    ...
    参数:
        ...
        train_dir: 训练集输出目录
        val_dir: 验证集输出目录
        val_ratio: 验证集比例（从train中抽样），默认0.1
        seed: 随机种子，确保可复现
    """
    random.seed(seed)
    ...
    print("\n" + "=" * 70)
    print(f"步骤 2/3: 划分数据集 (训练集:100% / 验证集:{val_ratio * 100:.0f}%采样)")
    print("=" * 70)

    # 划分策略：所有图片进train，随机抽样进val
    train_files = defaultdict(list)  # 所有文件
    val_files = defaultdict(list)  # 抽样文件
    stats = defaultdict(lambda: {'train': 0, 'val': 0})

    for category_name, image_files in sorted(grouped.items()):
        # 所有文件都进train
        train_files[category_name] = image_files.copy()

        # 从中随机抽样val_ratio比例进val
        image_files_copy = image_files.copy()
        random.shuffle(image_files_copy)

        n_total = len(image_files_copy)
        n_val = max(1, int(n_total * val_ratio))  # 至少1个样本进val

        val_files[category_name] = image_files_copy[:n_val]

        stats[category_name]['train'] = len(train_files[category_name])
        stats[category_name]['val'] = len(val_files[category_name])

    total_train = sum(stats[cat]['train'] for cat in stats)
    total_val = sum(stats[cat]['val'] for cat in stats)

    print(f"划分完成:")
    print(f"  训练集: {total_train} 张 (100%)")
    print(f"  验证集: {total_val} 张 ({total_val / total_train * 100:.1f}%)")
    ...
    # 复制训练集（所有文件）
    print("正在复制训练集文件（所有图片）...")
    for category_name, image_files in train_files.items():
        ...
        for img_file in image_files:
            ...
            if copied_train % 500 == 0:
                print(f"  进度: {copied_train}/{total_train} ({copied_train / total_train * 100:.1f}%)")

    print(f"  ✅ 训练集复制完成: {copied_train} 个文件")

    # 复制验证集（抽样文件）
    print("\n正在复制验证集文件（随机抽样）...")
    for category_name, image_files in val_files.items():
        ...
        for img_file in image_files:
            ...
            if copied_val % 500 == 0:
                print(f"  进度: {copied_val}/{total_val} ({copied_val / total_val * 100:.1f}%)")
    ...
    print("✅ 全部完成!")
    print("=" * 70)
    print(f"训练集: {copied_train} 张 (100%)")
    print(f"验证集: {copied_val} 张 ({copied_val / copied_train * 100:.1f}% 采样)")
